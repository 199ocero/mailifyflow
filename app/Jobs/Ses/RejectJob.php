...
class RejectJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($campaignLog) {
            $campaignLog->status = CampaignLogStatusType::REJECTED->value;
            $campaignLog->rejected_at = Carbon::parse($message['mail']['timestamp']);
            $campaignLog->reason_failed = $message['reject']['reason'];
            $campaignLog->save();
        }
    }
}
