...
class RenderingFailureJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($campaignLog) {
            $campaignLog->status = CampaignLogStatusType::RENDERING_FAILURE->value;
            $campaignLog->rendering_failure_at = Carbon::parse($message['mail']['timestamp']);
            $campaignLog->reason_failed = $message['failure']['errorMessage'];
            $campaignLog->save();
        }
    }
}
