...
<div class="flex w-full {{ $position }}">
    <a href="{{ $url }}" target="{{ $target }}"
        class="{{ $buttonColor }} relative grid-flow-col items-center justify-center font-semibold outline-none transition duration-75 focus-visible:ring-2 rounded-lg gap-1.5 px-3 py-2 text-sm inline-grid shadow-sm  text-white no-underline">
        {{ $label }}
    </a>
</div>
